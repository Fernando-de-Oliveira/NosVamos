...
class EnderecoController {
    ...
    public async getOne(req: Request, res:Response ): Promise<any>{
        const {id} = req.params;
        const endereco = await pool.query('SELECT * FROM endereco where id_endereco = ?', [id])
    ...
    public async update (req: Request, res: Response):Promise<void>{
        const {id} =req.params;
        await pool.query("UPDATE endereco set ? WHERE id_endereco = ?", [req.body, id])
        res.json({message: "O endereco foi editado"})
    }

    public async delete(req: Request, res: Response): Promise<void>{
        const {id} = req.params;        
        await pool.query('DELETE FROM endereco WHERE id_endereco = ?', [id]);
